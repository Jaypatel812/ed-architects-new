...
export function DataTable({
    ...
    const table = useReactTable({
        ...
        state: {
            globalFilter,
            pagination // ✅ use controlled state
        },
    ...
    const virtualRows = useVirtualScroll ? rowVirtualizer.getVirtualItems() : null

    const totalSize = virtualRows?.length ? rowVirtualizer.getTotalSize() : 0

    const paddingTop = virtualRows?.length ? virtualRows[0].start : 0
    const paddingBottom = virtualRows?.length ? Math.max(0, totalSize - virtualRows[virtualRows.length - 1].end) : 0
    ...
    return (
        <div className="w-full">
                                ...
                                {virtualRows.map((virtualRow) => {
                                    ...
                                    return (
                                        <tr
                                            ...
                                            className={cn(
                                                enableStriped && virtualRow.index % 2 === 0 && "bg-zinc-50",
            ...
            {!useVirtualScroll && enablePagination && <TablePagination table={table} />}
        </div>
    )
}
